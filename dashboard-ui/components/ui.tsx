...
export function Button({ children, variant = "subtle", className = "", type = "button", disabled = false, onClick }: { children: React.ReactNode; variant?: keyof typeof buttonStyles; className?: string; type?: "button" | "submit" | "reset"; disabled?: boolean; onClick?: () => void }) {
  return <button type={type} disabled={disabled} onClick={onClick} className={`inline-flex min-h-10 items-center justify-center gap-2 rounded-[11px] px-4 text-sm font-medium transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#9b87f5] disabled:pointer-events-none disabled:opacity-40 ${buttonStyles[variant]} ${className}`}>{children}</button>;
}
...
export function SectionTitle({ icon: Icon, title, description, action }: { icon?: LucideIcon; title: string; description?: string; action?: React.ReactNode }) {
  return <div className="mb-4 flex items-start justify-between gap-3"><div className="flex gap-3">{Icon && <div className="icon-glass mt-0.5 flex h-8 w-8 shrink-0 items-center justify-center rounded-[10px] text-[#b8acff]"><Icon size={16} /></div>}<div><h2 className="font-medium text-white">{title}</h2>{description && <p className="mt-1 text-xs text-zinc-500">{description}</p>}</div></div>{action}</div>;
}
