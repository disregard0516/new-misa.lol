...
function AddSocialModal({ open, initial, onClose }: { open: boolean; initial: SocialLink | null; onClose: () => void }) {
  ...
  const save = () => {
    ...
    if (displayMode === "link") {
      ...
      catch { setError("Enter a valid URL, like youtube.com/demo."); return; }
    }
    updateConfig((current) => {
  ...
  return <Modal open={open} title={initial ? `Edit ${initial.platform}` : `Add ${platform}`} description="Links are saved to your Misa.lol profile." onClose={onClose}>
