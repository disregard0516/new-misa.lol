...
/// Deletes a user; profiles, badges, entitlements and reports follow through ON DELETE CASCADE / SET NULL.
...
/// Pages whose owners opted in to being listed (config.settings.listed = true), newest first; suspended accounts excluded.
pub async fn list_listed_profiles(pool: &PgPool, limit: i64) -> Result<Vec<ListedProfile>, sqlx::Error> {
    sqlx::query_as::<_, ListedProfile>(
        "SELECT p.user_id, u.username, p.config, p.updated_at
         FROM profiles p JOIN users u ON u.id = p.user_id
         WHERE u.username IS NOT NULL AND u.suspended_at IS NULL
           AND p.config -> 'settings' ->> 'listed' = 'true'
