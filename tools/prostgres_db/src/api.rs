...
pub async fn list_profiles(State(state): State<AppState>, Query(query): Query<ProfilesQuery>) -> Response {
    if query.listed != Some(true) {
        return (StatusCode::BAD_REQUEST, Json(json!({"error": "only listed=true is supported"}))).into_response();
    }
    match db::list_listed_profiles(&state.pool, query.limit.unwrap_or(24)).await {
        Ok(rows) => (StatusCode::OK, Json(json!({"profiles": rows}))).into_response(),
        Err(error) => db_error(error),
    }
}
