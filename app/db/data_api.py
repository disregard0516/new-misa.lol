...
async def delete_user(user_id: str) -> bool:
    """Deletes the account; the profile, badges, entitlements and sessions' owner go with it (cascades in prostgres_db)."""
    response = await _client_or_raise().request("DELETE", f"/v1/users/{user_id}")
    if response.status_code == 404:
        return False
    response.raise_for_status()
    return True
...
async def list_listed_profiles(limit: int = 24) -> list[dict]:
    """Opted-in pages (config.settings.listed = true) for the explore page: [{user_id, username, config, updated_at}]."""
    payload = await _request("GET", "/v1/profiles", params={"listed": "true", "limit": limit})
    return list((payload or {}).get("profiles") or [])
