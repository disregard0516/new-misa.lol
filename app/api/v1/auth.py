...
class SignupRequest(BaseModel):
    ...
    # optional: the name typed into a claim box on the site (misa.lol/<username>); validated + checked before the account is created
...
async def _issue_session(
    ...
    token, ttl = await create_session(user.id, remember)
...
async def _finish_oauth(
    ...
    *,
    ...
    destination = "/dashboard" if current_user else next_path
    try:
...
async def available(request: Request, username: str = Query(min_length=1, max_length=64)) -> dict:
    """Live check for the claim boxes and the sign-up form. Never more than what the signup itself would reveal."""
    await limit_auth(request, "available", limit=30, window_seconds=60)
    try:
        name = validate_username(username)
    except ValueError:
        return {"available": False, "reason": "invalid"}
    try:
        if await admin_db.is_reserved(name):
            return {"available": False, "reason": "reserved"}
    except (RuntimeError, OSError):
        pass
    try:
        taken = await data_api.find_user(username=name) is not None
    except Exception:
        return {"available": None, "reason": "unknown"}
    return {"available": not taken, "reason": "taken" if taken else None, "username": name}
...
async def signup(payload: SignupRequest, request: Request, settings: SettingsDep) -> JSONResponse:
    ...
    if payload.username and payload.username.strip():
        ...
        if await data_api.find_user(username=username):
    ...
    if username:
        try:
            ...
            pass  # someone grabbed it between the check and the write — the dashboard asks again
